...
function createMealCard(meal) {
  ...
  card.innerHTML = `
    ...
    <div class="meal-card-actions">
      ...
      <button class="meal-card-audit-btn" data-meal-id="${esc(meal.id)}" title="Audit with culinary-knowledge agent">Audit</button>
    </div>
    ${renderIngredientsList(effective, { mealId: meal.id })}
  `;
  bindIngredientsToggle(card);
  const auditBtn = card.querySelector('.meal-card-audit-btn');
  if (auditBtn) {
    auditBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      auditMealViaChat(meal);
    });
  }
  card.addEventListener('click', (e) => {
    if (e.target.closest('.meal-delete-btn')) return;
    if (e.target.closest('.nutrition-badge')) return;
    if (e.target.closest('.meal-card-audit-btn')) return;
...
function auditMealViaChat(meal) {
  if (!state.apiKey) {
    alert('Set your Anthropic API key in the chat panel first (bottom-right floating button).');
  ...
  if (!input) return;
  input.value = `Audit the meal "${meal.name}" (id: ${meal.id}) against the culinary principles. List any unit, portion, combination, or cooking-sense issues, each with a short reason.`;
  sendChat();
}
...
${typeof CULINARY_PRINCIPLES_TEXT === 'string' ? `\n---\nCULINARY PRINCIPLES (authoritative source for cooking-judgment questions like audits, portion sanity, and ingredient classification. Cite specific sections when applicable.):\n\n${CULINARY_PRINCIPLES_TEXT}` : ''}`;
}
...
// Classification is driven by data/ingredient-tags.json (generated by the
// culinary-knowledge subagent, embedded into culinary-data.js). If an ingredient
// is missing from the tag file, it is treated as uncategorized and skipped,
// with a banner in the modal prompting the user to regenerate tags.
const USER_BUCKETS = ['protein', 'starch', 'vegetable', 'fruit', 'other'];

function classifyIngredient(name) {
  if (!name) return null;
  const tags = (typeof INGREDIENT_TAGS !== 'undefined' && INGREDIENT_TAGS.ingredients) || {};
  const tag = tags[name.toLowerCase().trim()];
  if (!tag) return null;            // uncategorized — flagged in modal header
  if (tag.hide) return null;         // aromatics / seasonings are hidden
  if (USER_BUCKETS.includes(tag.bucket)) return tag.bucket;
  return 'other';
}

function isIngredientUncategorized(name) {
  if (!name) return false;
  const tags = (typeof INGREDIENT_TAGS !== 'undefined' && INGREDIENT_TAGS.ingredients) || {};
  return !tags[name.toLowerCase().trim()];
}
...
function getCustomMealBuckets() {
  ...
  for (const name of names) {
    ...
    } else if (isIngredientUncategorized(name)) {
...
function renderCustomMealBuckets() {
  ...
  const { buckets, uncategorized } = getCustomMealBuckets();
  const warningEl = document.getElementById('custom-meal-warning');
  if (warningEl) {
    if (uncategorized.length > 0) {
      warningEl.style.display = '';
      warningEl.textContent = `${uncategorized.length} ingredient${uncategorized.length === 1 ? '' : 's'} aren't categorized yet and won't appear below. Ask the culinary-knowledge agent to regenerate ingredient tags.`;
    } else {
